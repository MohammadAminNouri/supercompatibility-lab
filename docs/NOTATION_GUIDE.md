...
## Parent/daughter cycle notation

| symbol | meaning | unit / scale |
|---|---|---|
| `g_P` | reconstructed parent (B2 for the NiTi cycle) crystal orientation | proper rotation matrix |
| `S_P^(k)` | k-th proper parent crystal-symmetry rotation used to generate a daughter branch | proper rotation matrix |
| `R_cp` | daughter→parent crystal-frame orientation-relationship rotation | proper rotation matrix |
| `g_D^(k)` | k-th symmetry-distinct regenerated daughter (B19′ for NiTi) orientation | proper rotation matrix |
| `δ_i` | round-trip closure misorientation of measured daughter grain `i` | degrees |
| `d_GD` | minimum disorientation after reducing by the daughter symmetry group | degrees |
| `P_i` | reconstructed parent assigned to measured daughter grain `i` | parent ID |
| `k_i*` | regenerated branch with the smallest closure residual for daughter grain `i` | branch ID |
| `Δθ_j→k` | symmetry-reduced daughter orientation change from regenerated branch `j` to branch `k` | degrees |

For the dedicated NiTi natural/AQ quick setup, crystallographic indices are converted through the direct and reciprocal lattice bases before the OR is constructed. Thus `[101]B19′` is interpreted as a real monoclinic direct-lattice direction, not as the Euclidean Cartesian vector `(1,0,1)`.
